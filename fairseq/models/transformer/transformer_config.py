...
class TransformerConfig(FairseqDataclass):
    ...
    SA_attention_dropout: float = field(
        default=0.0, metadata={"help": "SA_attention_dropout"}
    )
    SA_image_dropout: float = field(
        default=0.0, metadata={"help": "SA_image_dropout"}
    )
    SA_text_dropout: float = field(
        default=0.0, metadata={"help": "SA_text_dropout"}
    )
    vqa_SA_attention_dropout: float = field(
        default=0.1, metadata={"help": "vqa_SA_attention_dropout"}
    )
    vqa_SA_image_dropout: float = field(
        default=0.1, metadata={"help": "vqa_SA_image_dropout"}
    )
    vqa_SA_text_dropout: float = field(
        default=0.0, metadata={"help": "vqa_SA_text_dropout"}
    )
    
    # MMT
    use_single_encoder: bool = field(
        default=False, metadata={"help": "使用单流MMT encoder"}
    )
    use_dual_encoder: bool = field(
        default=False, metadata={"help": "使用双流MMT encoder"}
    )
    dual_encoder_freeze_vision_model: bool = field(
        default=False, metadata={"help": "冻结视觉模块"}
    )
    dual_encoder_freeze_text_model: bool = field(
        default=False, metadata={"help": "冻结文本模块"}
    )
    float16: bool = field(
        default=False, metadata={"help": "开不开fp16"}
    )
